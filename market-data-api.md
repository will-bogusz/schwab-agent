...
Level Two book for Options

Whole

CHART\_EQUITY

Chart candle for Equities

All Sequence

CHART\_FUTURES

Chart candle for Futures
...
Get account activity information such as order fills, etc
...
ADMIN, LEVELONE\_EQUITY etc. Please see Service Names table above.

command

Command (required)

LOGIN, SUBS, ADD, UNSUBS, VIEW, LOGOUT
...
\`schwabClientCustomerId\` as found in GET User Preference endpoint

SchwabClientCorrelId

Client's session ID

\`schwabClientCorrelId\` as found in GET User Preference endpoint. Unique identifier value that is attached to requests and messages that allow reference to a particular transaction or event chain.
...
1.  SUBS A,B,C (fresh sub for LEVELONE\_EQUITIES)
2.  SUBS A (fresh sub for LEVELONE\_EQUITIES, previous SUBS of B,C are unsub'ed, only A is sub'ed)
...
1.  ADD A,B (fresh sub for LEVELONE\_EQUITIES)
...
This changes the field subscription for a particular service. It will apply to all symbols for that particular service.
...
ADMIN, LEVELONE\_EQUITY, etc. Please see Service Names table in section 5.
...
Client should reconnect and re-login with new token. Client to determine if failed logins are expected.
...
STREAM\_CONN\_NOT\_FOUND

No connection found for user or new session but no login request
...
BAD\_COMMAND\_FORMAT

Command fails to match specification

No

Client should investigate why a command is not formatted properly

22

FAILED\_COMMAND\_SUBS
...
FAILED\_COMMAND\_UNSUBS

Unsubscribe command could not be completed successfully

24

FAILED\_COMMAND\_ADD

Add command could not be completed successfully

25

FAILED\_COMMAND\_VIEW

View command could not be completed successfully

26

SUCCEEDED\_COMMAND\_SUBS

Subscribe command completed successfully
...
SUCCEEDED\_COMMAND\_UNSUBS

Unsubscribe command completed successfully

28

SUCCEEDED\_COMMAND\_ADD

Add command completed successfully

29

SUCCEEDED\_COMMAND\_VIEW
...
All data is streamed to the client and includes a sequence number. Data is not conflated by the streamer although the underlying source of the data may conflate.

Change

Only fields that clients are interested in, and have changed, are streamed to the client. Data is conflated by the streamer.
...
Access token as found from POST Token endpoint.
...
Identifies the channel as found through the GET User Preferences endpoint.
...
Identifies the page or source in the channel where quote is being called from (5 alphanumeric).   
...
Correlation ID string passed by client
...
server=hostname-instance (for troubleshooting purposes)  
...
Schwab-standard symbols in uppercase and separated by commas  
e.g. AAPL,TSLA,IBM
...
Please see the LEVELONE\_EQUITIES Field Definition table below
...
_Notes, Examples Source_

key

String

Usually this is the symbol
...
Whether data is from the SIP or NFL

\- false : data is from a SIP   
...
ADR, CEF, COE, ETF, ETN, GDR, OEF, PRF, RGT, UIT, WAR
...
CUSIP number for the instrument, such as 594918104
...
Units are "lots" (typically 100 shares per lot)Note for NFL data this field can be 0 with a non-zero bid price which representing a bid size of less than 100 shares.
...
Please see the LEVELONE\_OPTIONS Field Definition table below
...
Descriptions are loaded from the database daily at 3:30 am ET.
...
Closing prices are updated from the DB at 7:29AM ET.
...
Volume is set to zero at 3:30am ET.
...
Volatility is reset to 0 at 3:30am ET
...
In-the-money is positive, out-of-the money is negative.
...
Yes The difference, measured in milliseconds, between the time an event occurs and midnight, January 1, 1970 UTC.
...
Please see the LEVELONE\_FUTURES Field Definition table below
...
_Update Regular Hours_

_Update AM/PM Hours_
...
Yes Currently "?" for unknown as all quotes are CME
...
Currently "?" for unknown as all quotes are CME
...
Minimum price increment of contract
...
Please see the LEVELONE\_FUTURES\_OPTIONS Field Definition table below
...
Please see the LEVELONE\_FOREX Field Definition table below
...
Minimum price increment for pair
...
NYSE\_BOOK, NASDAQ\_BOOK, OPTIONS\_BOOK
...
Please see the BOOK Field Definition table below
...
Equities symbols in upper case and separated by commas.  
e.g.: AAPL,TSLA,IBM  
 
...
Please see the CHART\_EQUITY Field Definition table below
...
_**Update Regular Hours**_

_**Update AM/PM Hours**_

_**Notes, Examples Source**_
...
Please see the CHART\_FUTURES Field Definition table below
...
SCREENER\_EQUITY, SCREENER\_OPTION
...
-   Indices: $COMPX $DJI, $SPX, INDEX\_ALL
-   Exchanges: NYSE, NASDAQ, OTCBB, EQUITY\_ALL
-   Option: OPTION\_PUT, OPTION\_CALL, OPTION\_ALL

  
and sortField is:

-   VOLUME, TRADES, PERCENT\_CHANGE\_UP, PERCENT\_CHANGE\_DOWN, AVERAGE\_PERCENT\_VOLUME
...
Please see the SCREENER Field Definition table below
...
Subscribed or requested symbol
...
Refer to the field table below
...
A client-provided string that streamer will populate updates with. Only first key is used if multiple are provided.
